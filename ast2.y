...
const char *ast_node_type_to_string(ASTNodeType type) {
    switch (type) {
        ...
        case AST_DECLARATION_GROUP:        return "DeclarationGroup";
        default:                         return "Unknown";
    }
}

vector<struct ASTNode *> splice_list(struct ASTNode *list) {
    ...
    for (int j = 1; j < node->child_count; j++) {
        ...
        if (decl_kid->type == AST_LIST) {
            for (int i = 0; i < decl_kid->child_count; i++) {
                struct ASTNode *init = decl_kid->children[i];
                // debug print
...
struct ASTNode *flatten_ast(struct ASTNode *node) {
    ...
    for (int i = 0; i < node->child_count; i++) {
        ...
        if (kid->type == AST_LIST) {
            ...
            printf("flatten_ast: hit DECL with %d kids\n", kid->child_count);
            auto decls = flatten_decl(kid);
            printf("flatten_ast: flatten_decl returned %zu nodes\n", decls.size());
            for (auto *d : decls)
                newKids.push_back(d);
        }
        else {
